...
% coupled_logistics_poisson_simulation_with_bottleneck2(2,4.46e8,4.46e8,8,20,20,10)
%
%% Implementation
% *main function*
...
function output_data = coupled_logistics_poisson_simulation_with_bottleneck2(rconstWT,...
...
%         plot(temp_nMut)
        ...
        cycles = 0; %define 100 cycles as survival threshold and extinction when WT accounts for 99.9% of population
       while cycles <= 100 && temp_nWT(end)/(temp_nWT(end)+temp_nMut(end))<0.999
        %bottleneck
...
function [nWT,nMut,time] = runVerhulst2(nWT0,nMut0,t0)
    ...
    while steps<nsteps && nWT(end)>0 && nMut(end)>0
        ...
        time = [time time(end)+doubling_time];
        steps = steps + 1;
    end    
...
%     time = t0;
    ...
    nWT = [nWT tempWT];
    nMut = [nMut tempMut];
    time = [time time(end) + n*doubling_time]; 
